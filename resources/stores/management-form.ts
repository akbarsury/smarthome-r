...
> {
    ...
    // method
    useForm = <T extends P>(field: T) => this.form.defineField(ref(field))
    useField = <T extends P>(field: T) => this.form.defineField(ref(field))

}

export const useManagementForm = defineStore('management-form', () => {
    const create = <FormName extends ValidationSchemaKey>(name: FormName) => new Form(name)
